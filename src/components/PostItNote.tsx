...
export default function PostItNote({ postIt, onUpdate, onRemove }: PostItNoteProps) {
  ...
  // Sync with store when postIt changes externally
  useEffect(() => {
    if (!isDragging) {
  ...
  const handlePointerMove = useCallback((e: React.PointerEvent) => {
    ...
    const clampedX = Math.max(0, Math.min(newX, parentRect.width - postIt.width));
    const clampedY = Math.max(0, Math.min(newY, parentRect.height - postIt.height));
  ...
  return (
    <motion.div
      ref={elRef}
      className="absolute postit-shadow select-none rounded-sm"
      style={{
        left: pos.x,
        top: pos.y,
        width: postIt.width,
    ...
    >
      {/* Top tape effect */}
      <div
        className="absolute -top-2 left-1/2 -translate-x-1/2 w-14 h-5 rounded-sm opacity-60"
        style={{
          background: 'linear-gradient(180deg, rgba(255,255,200,0.7), rgba(255,255,180,0.4))',
